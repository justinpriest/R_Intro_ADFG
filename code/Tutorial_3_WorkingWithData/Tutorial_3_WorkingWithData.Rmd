...
```{r show-dir, exercise=TRUE, exercise.lines = 5}
...
Now write the R code required to change your working directory:

```{r set-dir, exercise=TRUE, exercise.lines = 5}
...
```{r use-dplyr, exercise=TRUE, exercise.eval=TRUE, exercise.lines = 5}
library(dplyr)
exampledf
```
...
```{r check-structure, exercise=TRUE, exercise.eval=TRUE}
exampledf
...
Fill in the blanks within the function `as.factor()` to make columns `sex` and `statarea` both factors. 
```{r make-factor, exercise=TRUE}
...
Pretend that there is a file named "managementsurveys_2019.csv" in the folder `data` under your working directory. Import this file. 
...
*Now check your knowledge to make sure that you are retaining it. This is only self-scored.*
