...
Add a geom for a histogram (use "Help" in RStudio, google, or auto-complete feature). Complete the `fill =` for each penguin species.  
...
Set transparency of the histogram to 50% (0.5)
...
Manually adjust the scale of the fill values to be "darkorange", "purple", and "cyan4". This can be a tricky step! Google or ask for help if you're stuck.
...
Add the same species colors manually as in previous exercises. Add the minimal theme. Add labels using `labs()` for x, y, and color. Add the legend inside the plot at `c(0.85, 0.15)`. Hint: look at "Flipper Plot, Exercise 6".
...
View the data and fill in the aes() with the correct variables. Change the line size to 1.5
...
```{r lineplot-troubleshoot, exercise.eval=FALSE}
...
*To tell ggplot to plot by species, within *`facet_wrap()` *write the tilde (~), followed by the variable. There is also * `facet_grid()` *where you can specify the number of columns or rows.*
